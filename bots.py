...
def _get_move_from_expectationis(game, my_exp, bluff=0):
    ...
    state_likelyhoods = {(1, 0): -(my_exp[game.state[1] - 1] - game.state[0])}
    state = game.get_next_state(game.state)
...
def bot_best_expectation(game, bluff=0):
    ...
    my_exp = [game.get_my_dice().count(i) 
              ...
              + (2 - i // 6) / 6 * (
                      len([item for sublist in game._dice for item in sublist]) - len(game.get_my_dice()))
              for i in range(1, 7)]
...
def bot_best_expectation_with_hist(game, bluff=0):
    ...
    my_exp = [game.get_my_dice().count(i)
              + game.get_my_dice().count(6)
              + dice_guess[i - 1] * 2 / 6
              + (2 - i // 6) / 6 * ((sum(game.dice_nr)) - len(game.get_my_dice())- len(dice_guess))
              for i in range(1, 7)]
